...
export default function ReportLostPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      fd.append('dateLost', form.dateLost);
  ...
  return (
    <div className="mx-auto max-w-lg">
      <div className="glass-card p-6">
        <h1 className="font-display text-2xl font-bold text-clay-ink">Report a lost item</h1>
        <p className="text-sm text-clay-muted">You will receive a unique ID and status &quot;Looking&quot;.</p>
        {formError && (
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <input
              className={`input-field ${fieldErrors.dateLost ? 'input-error' : ''}`}
              type="datetime-local"
              required
              max={maxDatetimeLocalValue()}
              value={form.dateLost}
              onChange={(e) => patchForm({ dateLost: e.target.value })}
            />
            {renderError('dateLost')}
          </div>
          <div>
          ...
          <div>
            <label className="label">Event (optional — attended events only)</label>
            <select
              className="input-field"
              value={form.event}
